...
class DRL(object):
    ...
    def extract_features(self, valid_pairs, task_instance_features):
        ...
        for machine, task in valid_pairs:
            ...
            features.append([machine.cpu, machine.memory/1024, machine.mips, machine.price, task.task_config.memory/machine.mips] + self.features_extract_func(task, task_instance_features))
        return features
    ...
    def global_features(self, cluster, clock):
        ...
        features = [cluster.cpu, cluster.memory/1024, cluster.mips, len(cluster.running_task_instances), 
                    len(cluster.finished_tasks), len(cluster.ready_unfinished_tasks), clock]
        return features
    ...
    def __call__(self, cluster, clock):
        ...
        # 选择动作概率密度
        action_logits, values = self.agent.select_action(features, temperature=1)
